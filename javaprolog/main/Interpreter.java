...
public class Interpreter {
	...
	private class ActionVisitor implements IActionVisitor<Goal, Set<WorldObject>> {
		...
		/**
		 * TODO: should move retain relationships? E.g.:
		 * "move (a ball on a box) on a table". Should the ball still be on a
		 * box afterwards?
		 * 
		 * @param n
		 * @param worldObjects
	...
	private class NodeVisitor implements INodeVisitor<LogicalExpression<WorldObject>, Set<WorldObject>, Quantifier> {
		...
		public LogicalExpression<WorldObject> visit(RelativeEntityNode n, Set<WorldObject> worldObjects,
			...
			if (q.equals(Quantifier.THE)) {
				...
				if (wobjs.size() > 1) {
					if (!Shrdlite.debug) {
						throw new InterpretationException("Several objects match the description '"
								+ n.getObjectNode().getChildren().toString() + "' with relation '"
								+ n.getLocationNode().getRelationNode().getRelation() + "' to '"
								+ n.getLocationNode().getEntityNode().getChildren().toString()
								+ "'. Which one do you mean?");
					}
				} else if (wobjs.isEmpty()) {
					if (!Shrdlite.debug) {
						throw new InterpretationException("There are no objects which match the description '"
								+ n.getObjectNode().getChildren().toString() + "' with relation '"
								+ n.getLocationNode().getRelationNode().getRelation() + "' to '"
								+ n.getLocationNode().getEntityNode().getChildren().toString());
					}
				}
				LogicalExpression<WorldObject> le = new LogicalExpression<WorldObject>(wobjs,
				...
				// to). TODO: As it is now, all bricks must be on a table after
				// the planner finishes. There are two interpretations!
				// Ambiguous!
				return world.attachWorldObjectsToRelation(matchesArg1, matchesLocation);
			}
			// Set<WorldObject> wobjs =
			// world.filterByRelation(matchesArg1.getObjs(), matchesLocation,
			// LogicalExpression.Operator.OR);
			...
			// n.getLocationNode().getEntityNode().getChildren().toString());
			// }
			// LogicalExpression<WorldObject> le = new
			// LogicalExpression<WorldObject>(wobjs,
		 ...
		 * @param n
		 * @param worldObjects
		 *            if null, all objects in the world are used
		 * @param dummy2
		 * @return
		 * @throws InterpretationException
		 */
		...
		public LogicalExpression<WorldObject> visit(RelativeNode n, Set<WorldObject> worldObjects, Quantifier dummy2)
			...
			if (relativeTo.isEmpty() && !Shrdlite.debug) {
				throw new InterpretationException("There are no objects which match the description '"
						+ n.getEntityNode().getChildren().toString() + ".");
			}
		...
		public LogicalExpression<WorldObject> visit(ObjectNode n, Set<WorldObject> worldObjects, Quantifier quantifier)
			...
			// throw new
			// InterpretationException("Several objects match the description '"
			// + n.getChildren().toString() +
			// "'. Which one do you mean?");//TODO: Proper error message
			// } //This is actually OK. Consider the sentence
			// "["take", "the", "box", "under", "an", "object", "on", "a", "green", "object"]".
			// "The box" can be several boxes..
