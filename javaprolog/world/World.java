		...
		if (goal.getAction().equals(Goal.Action.TAKE)) {
			...
			if (op.equals(LogicalExpression.Operator.OR)) {
				for (WorldObject wo : s) {
					if (wo.getId().equals(holding.getId()))
						return true;
				}
			}
			return (s.size() != 1)
					&& holding.getId().equals(s.iterator().next().getId());
		}
		if (op.equals(LogicalExpression.Operator.AND)) {
			return s.size() == (le.getObjs() == null ? 0 : le.getObjs().size())
					+ le.getExpressions().size(); // TODO: for this to work,
